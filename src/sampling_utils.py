...
def uniform_times_by_day(days, n=500):
    res = OrderedDict()
    for day in days:
        date_tuple = datetime.date(day.year,day.month,day.day)
        time_min = datetime.datetime.combine(*date_tuple, datetime.time.min)
        time_max = datetime.datetime.combine(*date_tuple, datetime.time.max)
        res[day] = np.random.rand(n) * (time_max-time_min) + time_min
    return res

def uniform_locations_by_state(states, n=500):
    res = OrderedDict()
...
def sample_time_and_space(data, times_by_day, locations_by_state):
    n_total = data.sum().sum()
    t_all = np.empty((n_total,), dtype=object)
    x_all = np.empty((n_total,2))
    
    i = 0
    for (state_id, series) in data.iteritems():
        for (day, n) in series.iteritems():
            ...
            locs = locations_by_state[state_id]
...
class IAEffectSampler(object):
    def __init__(self, data, times_by_day, locations_by_state, temporal_bfs, spatial_bfs, num_tps = 10, time_horizon=5, verbose=True):
        self.ia_bfs = build_ia_bfs(temporal_bfs, spatial_bfs)
        self.times_by_day = times_by_day
        self.locations_by_state = locations_by_state
